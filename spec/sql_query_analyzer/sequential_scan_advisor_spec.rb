...
RSpec.describe SqlQueryAnalyzer::SequentialScanAdvisor do
  before(:each) do
    ...
    ActiveRecord::Base.connection.add_index :test_users, [:name, :active]
  end
  ...
  describe "private methods" do
    ...
    describe "#extract_table_and_columns" do
      it "extracts table name and columns correctly" do
        ...
        expect(columns).to match_array(["name", "active"])
      end
    end

    describe "#missing_composite_index?" do
      it "returns false when index exists" do
        expect(advisor.send(:missing_composite_index?, "test_users", ["name", "active"])).to be false
      end

      it "returns true when index does not exist" do
        expect(advisor.send(:missing_composite_index?, "test_users", ["email", "active"])).to be true
      end
    end
  end
end 